...
export default function Act6_UnpaidDebt({ className, selectedCountry }: Act6Props) {
  ...
  return (
    <section
      id="unpaid-debt"
      ref={sectionRef}
      aria-label="Act VI: Unpaid Debt narrative"
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-6 md:px-12 flex flex-col-reverse md:flex-row">
        ...
        <div role="region" aria-label="Climate adaptation tax chart simulator" className="w-full md:w-7/12 h-screen sticky top-0 flex flex-col justify-center items-center py-12 md:py-0 md:pl-12 z-0">
          <div ref={rightColumnRef} className="w-full max-w-4xl relative h-[60vh] flex flex-col justify-center items-center">
            
            <TaxChart activeStep={activeStep} selectedCountry={selectedCountry} />
            ...
            <div className="absolute bottom-[-52px] left-0 right-0 text-center text-[9px] uppercase tracking-widest font-body text-drift-wood/75">
              {getStation(selectedCountry?.id, 'tax').stationName} — {getStation(selectedCountry?.id, 'tax').dataSource} (% GDP)
            </div>
